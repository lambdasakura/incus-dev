...
| 版 | 追加されたもの |
| --- | --- |
| `1.1` | `workspace` の複数形式（3.7.2） |
| `1.2` | `workspace.owner`（3.7.3） |

いずれも使わないdev.ymlは、それより前の版を宣言したままでよい。
...
既定でコンテナ側は0（root）である。`workspace.owner` で別のidを指定できる。

```yaml
workspace:
  idmap: raw
  owner:
    uid: 1000        # コンテナ内のこのidがホストの実行ユーザーになる
    gid: 1000        # 省略時は uid と同じ
```

これは `raw.idmap` のコンテナ側を書き換える。

```text
raw.idmap: uid <host uid> 1000
            gid <host gid> 1000
```

`shell.user` に一般ユーザーを指定して作業する構成のためにある。
既定（0）ではworkspaceがコンテナ内からroot所有に見えるため、
root以外のアカウントはそもそも書き込めない（3.7.3の表）。
`owner` を指定すると、workspaceはそのidの所有として見え、
そのアカウントが書いたファイルはホストの実行ユーザーの所有になる。

**rootは同時には写せない。** 実daemonで確認したところ、
1つのホストidを2つのコンテナidへ写す `raw.idmap` を与えたinstanceは起動しない。
したがって `owner` を指定すると、コンテナのrootが書いたファイルは
ホストではsubuidの所有になる。provisioningはrootで走るため、
provisioningがworkspaceへ書いたファイルは実行ユーザーのものにならない。
どちらを「自分」にするかの選択であり、両方は取れない。

値は数値のみとする。`raw.idmap` はinstance作成時に決める必要があり、
その時点でコンテナ内に当該アカウントはまだ存在しないため、
名前を引くことができない。

`idmap: raw` と `idmap: auto` でのみ指定できる。
`shift` はコンテナのuid Nをホストのuid Nへ写すもので、写す先を選べない。
`none` は何も写さない。どちらも `owner` を指定した場合はエラーとする。
黙って無視すると、指定した側は効いていると思い込む。

`auto` が `shift` へ退避した場合、`owner` は適用されない。
退避を知らせる警告がそのことも述べる。

`/etc/subuid` / `/etc/subgid` に要る許可は変わらない。
必要なのはホスト側idの使用許可であり、写す先のidには要らない。
...
上の表は **コンテナ内のrootが作ったファイル** についてのものである。
root以外のアカウントで作業する場合、対応付けは逆の端を写す。

どちらも「何もしない状態」からの差分として読む必要がある。
ホスト uid 1000 の環境で実daemonに対して測った値を並べる。

| 値 | コンテナのidマップ | root が書いたもの | uid 1000 が書いたもの |
| --- | --- | --- | --- |
| `none` | uid 0 → host 1000000 | host 1000000 | host 1001000 |
| `raw` | uid 0 → host **1000**、uid 1〜 はそのまま | host **1000** | host 1001000 |
| `shift` | `none` と同じ | host **0** | host **1000** |

**`raw` はマウントを翻訳しない。** `raw.idmap: uid 1000 0` は
「ホストの1000をコンテナの0へ」という幅1のエントリをidマップへ挿す指定であり、
書き換わるのはコンテナ uid 0 だけである（右列が `none` と一致する）。
効果はworkspaceに限らずコンテナ全体に及ぶ。
`/etc/subuid` に要るのが `root:<uid>:1` の1個分なのはこのためである。

**`shift` はidマップを変えず、そのマウントで名前空間の効果を打ち消す。**
idマップは `none` と同じままで、マウントを通る全てのidから 1000000 を引く。
コンテナ uid N がホスト uid N に見えるのは、
2つの翻訳が打ち消し合った結果であって、翻訳をしていないからではない。

この違いが一般アカウントでの作業を左右する。`raw` ではworkspaceが
コンテナ内からrootの所有に見えるため、root以外のアカウントは書き込めない。
`shift` ではアカウントに実行ユーザーと同じuid/gidを与えれば、
そのアカウントが書いたファイルは実行ユーザーのものになる
（[10-examples.md](10-examples.md) 10.3.1）。
