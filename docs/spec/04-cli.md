...
その実行に限り `shell.user` を上書きする。`idev exec` にも同じフラグを持たせる。
...
`shell.user` は「普段どのユーザーで入るか」の宣言であり、
プロジェクト全員に共通する。一方で管理作業のために一度だけ別のユーザーで
入りたいことがあり、そのために `dev.yml` を書き換えるのは、
...
したがって `idev exec -- /bin/sh` は対話シェルにならない。
シェルから見て端末が無いため、プロンプトも職制御も無い。
これは欠陥ではなくこの規定の帰結であり、対話シェルは `idev shell` が担う。

実行ユーザー・作業ディレクトリの扱いは `idev shell` と同じ（`shell` の指定に従い、
`--user` で上書きできる）。
...
instanceが動いている場合、割り当てられているglobalアドレスを表示する。

```text
Addresses:  10.216.13.42, fd42:1:2:3::1
```

先頭は `idev ip` が返すものと同じである（4.4.1）。
`--json` では interface 名と family も返す。
...
止まっているinstanceにアドレスは無い。行そのものを出さない。
「なし」と書くと、動いていないのか、動いていてアドレスが付いていないのかの
区別がつかない。
...
instanceのアドレスを1つ、標準出力へ出す。

**ラベルも装飾も付けない。** この出力はコマンド置換に入るために存在する。
`$(...)` は末尾の改行を落とすので、1行で出す。

### どのアドレスを返すか

globalアドレスのみを対象とする（loopbackとlink-localは除く）。
そのうえで次の順で選ぶ。

1. IPv4（`inet`）を IPv6（`inet6`）より優先する
2. 同じfamilyが複数ある場合、interface名の辞書順で最初のもの

Incusの既定bridgeではIPv6（ULA）が先に付き、IPv4のDHCPが終わるまで
外へ出られない。IPv4を優先するのはそのためである（[03-configuration.md](03-configuration.md) 3.7.3）。

順序を決めておくのは、NICが複数あるinstanceで実行のたびに違う値が返ると、
`$(idev ip)` を使う側が気づけないまま別のホストへ繋ぎにいくためである。

複数のアドレスが要る場合は `idev status --json` を使う。
`idev ip` に一覧を出す形式は持たせない。1行1件の出力と
「1つを返す」出力を1つのコマンドが兼ねると、
呼び出し側が `$(...)` で受けたときに黙って複数行を掴む。

### 返せない場合

いずれも標準エラー出力へ理由を書き、非0で終了する。
**標準出力へは何も書かない。** 空文字を返すと `ssh user@` が
ローカルユーザーへ繋ごうとする。

| 状態 | 言うこと |
| --- | --- |
| instanceが無い | `idev up` を促す |
| 動いていない | 動いていないこと（アドレスは動いている間だけ存在する） |
| 動いているがアドレスが無い | NICが無いか、DHCPがまだであること |

待たない。`idev up` が起動時にアドレスを待つ（[05-incus.md](05-incus.md)）ので、
このコマンドは読み取りに徹する。待つ読み取りは
`$(idev ip)` を書いた側から見て、固まったのか失敗したのか分からない。
...
- **標準出力**はコマンドの結果だけを載せる。
  `status` / `validate` / `provision --list` / `snapshot list` / `up --dry-run` / `ip` の出力と、
