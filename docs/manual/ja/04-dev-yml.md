...
ホストの実行ユーザーを、コンテナ内のどのidへ写すか。`raw` のときだけ指定できる。

```yaml
workspace:
  idmap: raw
  owner:
    uid: 1000        # コンテナ内のこのidがホストの自分になる
    gid: 1000        # 省略時は uid と同じ
```

省略時はroot。その場合workspaceはコンテナ内からroot所有に見えるため、
root以外のアカウントは書き込めない。`shell.user` で指定するアカウントのidを
`owner` に書けば、そのアカウントが書いたファイルがホスト側で自分の所有になる。

**rootとそのアカウントの両方を自分にすることはできない。**
1つのホストidは1つのコンテナidへしか写せず、両方を要求したinstanceは起動しない。
そのため `owner` を指定すると、コンテナがrootとして書いたファイルは
自分の所有にならないsubuidになる。provisioningはrootで走るので、
provisioningがworkspaceへ書いたものは自分のものにならない。

値は数値のみ。`raw.idmap` はinstance作成時に決める必要があり、
その時点でコンテナ内にそのアカウントはまだ無いため、名前を引けない。

`shift` と `none` では `owner` を拒否する。`shift` はコンテナのid Nを
ホストのid Nへ写すもので写す先を選べず、`none` は何も写さない。
`auto` は受け付けるが、`shift` へ退避した場合は適用できず、その旨を述べる。
